...
int main(){
	...
	for(int i=0; i<=monkey_index; i++){
		monkeys[i].items = malloc(sizeof(long long)*total_items);
	...
	*/
	...
	max1 = max2 = 0;
	for(int i=0;i<=monkey_index; i++){
		long long inspections = monkeys[i].inspections;
		if(inspections>max1){
			if(max1>max2)
				max2=max1;
			max1=inspections;
	...
	printf("PART TWO:%lld*%lld=%lld",max1,max2,max1*max2);
